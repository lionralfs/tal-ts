...
export class List extends Container {
  ...
  private createDataBoundItems(): void {
    ...
    const processDataError = response => {
      this.removeChildWidgets();
      this.bubbleEvent(new DataBoundEvent('databindingerror', this, null, response));
    };

    this.bubbleEvent(new DataBoundEvent('beforedatabind', this));
